...
public class TessBaseAPI {
    ...
    public boolean init(String datapath, String language, int ocrEngineMode) {
        //Log.d("chose init datapath ",datapath + "language  :" + language + "ocrEngineMode  :" +ocrEngineMode);
        if (datapath == null)
    ...
    private native boolean nativeNeedToFocus(byte[] src, int aiWidth, int aiHeight, int aiChannels);

    private native int nativeImageBlurDetect(byte[] src, int aiWidth, int aiHeight, int aiChannels);
    ...
    private byte[] bitmap2Bytes(Bitmap bitmap) {
        ByteBuffer BottomBuffer = ByteBuffer.allocate(bitmap.getByteCount());
        bitmap.copyPixelsToBuffer(BottomBuffer);
        return BottomBuffer.array();
    }

    public boolean NeedToFocus(Bitmap bmp, int aiWidth, int aiHeight, int aiChannels) {

        return nativeNeedToFocus(bitmap2Bytes(bmp), aiWidth, aiHeight, aiChannels);
    }

    public int ImageBlurDetect(Bitmap bmp, int aiWidth, int aiHeight, int aiChannels) {

        return nativeImageBlurDetect(bitmap2Bytes(bmp), aiWidth, aiHeight, aiChannels);
    }
}
