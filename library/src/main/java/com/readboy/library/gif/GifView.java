package com.readboy.library.gif;
...
public class GifView extends View implements GifAction {
     ...
     * @param gif 图片
     */
    synchronized public void setGifImage(byte[] gif){
    	mHandler.removeMessages(REDRAW_MSG_ID);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if(gifDecoder == null) {
        	Log.w(TAG, "***************************onDraw, gifDecoder is null");
        	return;
        }
        if(currentImage == null){
        	Log.w(TAG, "***************************onDraw, currentImage is null A");
        	currentImage = gifDecoder.getImage();
        }
        if(currentImage == null){
        	Log.w(TAG, "***************************onDraw, currentImage is null B");
        	return;
        }
        int saveCount = canvas.getSaveCount();
        canvas.save();
        canvas.translate(getPaddingLeft(), getPaddingTop());
        //Log.w(TAG, "***************************onDraw, showWidth: "+showWidth);
		if (rect == null) {
    ...
    public void parseOk(boolean parseStatus,int frameIndex){
    	if(parseStatus){
    		if(gifDecoder != null){
        		//Log.i(TAG, "***************************parseOk, frameIndex"+frameIndex+", frame count:"+gifDecoder.getFrameCount());
    			...
    			case COVER:
    				if(frameIndex == 1){
    					...
    					if(gifDecoder.getFrameCount() > 1){
    						Log.i(TAG, "***************************parseOk, redraw" + frameIndex + ", frame count:" + gifDecoder.getFrameCount());
    						Message msg = mHandler.obtainMessage(REDRAW_MSG_ID);
			...
			if(msg.what == REDRAW_MSG_ID) {
    			if(pause == false){
	    			if(gifDecoder.parseOk()){
	    				...
	    				long sp = frame.delay;
    					msg = mHandler.obtainMessage(REDRAW_MSG_ID);
						removeMessages(REDRAW_MSG_ID);
    					mHandler.sendMessageDelayed(msg, /*sp +*/ 500/*offset delay*/);
	    			}else{
...
//	};
    			...
    			if(pause == false){
	    			if(gifDecoder.parseOk()){
	    				...
	    				if(redrawHandler != null){
	    					Message msg = redrawHandler.obtainMessage();
	    					redrawHandler.sendMessage(msg);
	    					Log.i("DrawThread run", "******************** gif delay ***************" + sp);
    ...
    public void destroy(){
    	isRun = false;
    	mHandler.removeMessages(REDRAW_MSG_ID);
    	redrawHandler.removeMessages(redrawHandler.obtainMessage().what);
    	if(gifDecoder != null){
    		gifDecoder.freeRecyle();
    		gifDecoder = null;
    	}
    }
